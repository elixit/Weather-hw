...
function currentWeather(city){ // refers to user input
    ...
    $.ajax({
      ...
      //https://openweathermap.org/current used as reference for weather conditions
        var date=new Date(display.dt*1000).toLocaleDateString(); // attatches new date tolocal storage
        ...
        $(currentTemperature).html((tempF).toFixed(2)+"&#8457"); // degree farenheit symbol
...
function forecast(cityid){
    ...
    $.ajax({
        ...
        for (i=0;i<5;i++){
            var date= new Date((display.list[((i+1)*8)-1].dt)*1000).toLocaleDateString();
